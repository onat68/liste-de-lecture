  ...
  <div
    ...
    <transition name="div" mode="out-in">
      ...
      <div v-else-if="typePicker.opened" class="box-border flex w-fit relative gap-1.5 md:gap-2">
        <SearchTypeButton v-for="(type, index) in typesToDisplay()" :class="'relative'" :key="index" :type="type"
          @click="setType(type)"></SearchTypeButton>
      </div>
    </transition>
    <transition name="div">
      <router-view name="cancelbutton"></router-view>
    </transition>
    <label for="searchField" class="absolute invisible pointer-events-none">Search field</label>
    <searchField :id="'searchField'" v-model="search.query" :class="typePicker.currentType.focusColor"></searchField>
    <transition name="divrev">
      <SearchButton v-if="!typePicker.opened" :class='"relative"'
        @click="search.search(typePicker.currentType.typeName, query)"></SearchButton>
    </transition>
  </div>
</template>
