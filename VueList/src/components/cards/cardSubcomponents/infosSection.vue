...
<template>
  ...
  >
    <div
      class="TitleTypeWrapper self-stretch h-min gap-1 flex-row justify-between items-start flex"
    >
      <h3 class="Title text-base md:text-lg lg:text-xl self-stretch font-display font-bold leading-5 line-clamp-1">
        {{ title }}
      </h3>
      <p :class="typeStyles.color" class="Type text-base md:text-lg lg:text-xl font-medium leading-tight">
        {{ type }}
      </p>
    </div>
    <div
      class="AuthorInfosWrapper w-full text-xs md:text-base lg:text-lg leading-4 h-min font-medium flex flex-row justify-between gap-2 flex-wrap-reverse gap-y-0"
    >
